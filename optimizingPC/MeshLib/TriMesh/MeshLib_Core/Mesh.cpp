...
{
	maxVid = maxFid = -1;
	boundaryLabeled=false; 
	hasVPropertyIndex = hasFPropertyIndex = hasEPropertyIndex = hasHEPropertyIndex = false;
}

Mesh::Mesh(std::vector<Face*>& flist) {
	maxVid = maxFid = -1;
	boundaryLabeled=false; 
	hasVPropertyIndex = hasFPropertyIndex = hasEPropertyIndex = hasHEPropertyIndex = false;

	std::set<Vertex *> nvset;
	for (int i=0;i<flist.size(); ++i)
	{
		Face * f = flist[i];
		Halfedge * he = f->he();
		nvset.insert(he->source());
		nvset.insert(he->target());
		nvset.insert(he->next()->target());				

	}

	for (std::set<Vertex*>::iterator b = nvset.begin(), e = nvset.end(); b != e; ++b)
	{
		Vertex * ov = *b;
		Vertex * nv = createVertex(ov->id());
		
		nv->point() = ov->point();
		nv->PropertyStr() = ov->PropertyStr();
		id2Ver[nv->id()] = nv;
	}

	id2Ver.rehash(id2Ver.size());
	
	std::map< std::pair<int, int>, Edge* > pairEdgeMap;

	for (int i=0;i<flist.size();++i)
	{
		Face * f = flist[i];
		Face * nf = createFace(f->id());
		...
		for (int j = 0; j < 3; ++j) {
			Halfedge *che = nhe[j];
			int v1 = che->source()->id();
			int v2 = che->target()->id();
			std::pair<int, int> vpair = std::make_pair(std::min(v1, v2), std::max(v1, v2));
			Edge *edge = pairEdgeMap[vpair];
			if (!edge) {
				edge = createEdge(che, NULL);
				pairEdgeMap[vpair] = edge;
			} else {
				edge->he(1) = che;
			}
...
{	
	...
	while(!feof(fp)){	
		...
		if( !strcmp(str, "Vertex" ) ){ //parsing a line of vertex element
			...
			v->index() = v_adjHeList.size();
			v_adjHeList.push_back(heList); // the adjacent halfedges of vertex v is stored in v_adjHeList[v->index()]
...
void Mesh::copyTo( Mesh & tMesh )
{
	...
	tMesh.id2Ver.rehash(tMesh.id2Ver.size());
...
Halfedge * Mesh::vertexHalfedge(Vertex * srcV, Vertex * trgV)
{
	Halfedge * he0 = srcV->most_clw_out_halfedge();
	if(he0->target() == trgV)
		return he0;
	Halfedge * he = he0->ccw_rotate_about_source();
	while (he!=he0 && he){
...
Face * Mesh::idFace(const int id )
{
	...
	return (cpos != id2Face.end())?(cpos->second):NULL;
}
...
Vertex * Mesh::idVertex( int id ) 
{
	...
	return (cpos != id2Ver.end())?(cpos->second):(NULL);
}
...
{	
	...
	for(i = 0; i < 3; i ++ )
	{
		...
		hes[i]->target() = ver[i];
		ver[i]->he() = hes[i];
		std::vector<Halfedge *> & adjHeList = v_adjHeList[ver[i]->index()];
	...
	for( i = 0; i < 3; i ++ )
	{
		...
		Edge * e = NULL;
		std::vector<Halfedge *> & adjHeList = v_adjHeList[ev1->index()];
...
//(5) Indexing
void Mesh::SetVPropertyIndex()
{	
	if (hasVPropertyIndex)
		return;
	int ind=0;
	for (std::list<Vertex *>::iterator vit=m_verts.begin(); vit!=m_verts.end(); ++vit){
		Vertex * v=*vit;
		v->index()=ind++;
	}
	hasVPropertyIndex=true;
...
void Mesh::SetFPropertyIndex()
{	
	if (hasFPropertyIndex)
		return;
	int ind=0;
	...
	hasFPropertyIndex=true;
}

void Mesh::SetEPropertyIndex()
{
	if (hasFPropertyIndex)
		return;
	int ind=0;
	...
	hasEPropertyIndex=true;
}

void Mesh::SetHEPropertyIndex()	
{
	if (hasHEPropertyIndex)
		return;
	int ind=0;
	for (std::list<Edge *>::iterator it=m_edges.begin(); it!= m_edges.end(); ++it){
		Edge * e =*it;		
